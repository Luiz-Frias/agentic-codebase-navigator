...
class _BarrierLLM(LLMPort):
    ...
    If the broker regresses to sequential execution, this will deadlock and the
    broker timeout should kick in (failing the test quickly).
    """
...
def test_tcp_broker_complete_batched_runs_subcalls_concurrently_under_load() -> None:
    expected = 25
    ...
    try:
        ...
        assert [c.response for c in out] == [f"R(p{i})" for i in range(expected)]
    finally:
        broker.stop()

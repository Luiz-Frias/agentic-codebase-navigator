...
def test_local_environment_allowed_import_roots_overrides_policy() -> None:
    env = LocalEnvironmentAdapter(allowed_import_roots={"math"})
    try:
...
@pytest.mark.unit
def test_local_environment_rejects_none_execute_timeout() -> None:
    with pytest.raises(ValueError, match="execute_timeout_s"):
        LocalEnvironmentAdapter(execute_timeout_s=None)  # type: ignore[arg-type]


@pytest.mark.unit
def test_local_environment_rejects_execute_timeout_over_cap() -> None:
    with pytest.raises(ValueError, match="execute_timeout_s.*execute_timeout_cap_s"):
        LocalEnvironmentAdapter(execute_timeout_s=10, execute_timeout_cap_s=1)
...
def test_local_environment_llm_query_success_records_llm_calls() -> None:
    ...
    env = LocalEnvironmentAdapter(broker=broker)
    try:
...
def test_local_environment_cleanup_is_best_effort_and_idempotent(
    ...
    assert env._worker_process is None
    assert env._worker_conn is None
